...
const FACULTY_MEMBERS = [
    {
        ...
        badgeColor: "from-[#F0B429] to-[#804501]",
    ...
    },
    {
        ...
        badgeColor: "from-[#0B1E3D] to-[#804501]",
    ...
    },
    {
        ...
        badgeColor: "from-[#F0B429] to-[#0B1E3D]",
    ...
    },
    {
        ...
        badgeColor: "from-[#0B1E3D] to-[#F0B429]",
    ...
    },
    {
        ...
        badgeColor: "from-[#804501] to-[#0B1E3D]",
                    ...
                    <div className="p-7 md:p-10">
                        <div className="grid md:grid-cols-12 gap-8 items-start mb-8">
                            <div className="md:col-span-4">
                                <div className="relative mb-5">
                                    <div className="absolute -inset-1 rounded-2xl bg-gradient-to-br from-[#F0B429] to-[#804501] blur-md opacity-40" />
                                    <div className="relative w-full h-52 rounded-2xl overflow-hidden border border-white/12">
                                        <img src={faculty.image} alt={faculty.name} className="w-full h-full object-cover object-top" />
                                        <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between">
                                            <span className="px-2.5 py-1 rounded-full text-xs font-bold text-[#FDD34F] bg-[#0B1E3D]/80 border border-[#F0B429]/30">
                                ...
                                <div className="grid grid-cols-3 gap-2 text-center">
                                    {[
                                        ...
                                        { val: faculty.papers, label: "Papers", color: "text-[#F0B429]" },
                                        { val: faculty.courses, label: "Courses", color: "text-[#FDD34F]" },
                                    ].map((s, i) => (
                            ...
                            <div className="md:col-span-8">
                                <span className="text-xs font-black uppercase tracking-widest text-[#FDD34F] mb-2 block">{faculty.department}</span>
                                <h2 className="text-3xl font-black text-white mb-1 leading-tight">{faculty.name}</h2>
                                <p className="text-sm font-bold text-[#F0B429] mb-5">{faculty.designation}</p>
                                ...
                                <div className="flex items-center gap-3 mb-6 p-4 rounded-2xl bg-white/[0.04] border border-white/8">
                                    <div className="flex gap-0.5">
                                        {[...Array(5)].map((_, i) => <Star key={i} className="w-4 h-4 fill-[#F0B429] text-[#F0B429]" />)}
                                    </div>
                                ...
                                <h4 className="text-sm font-black text-white mb-3 flex items-center gap-2">
                                    <Award className="w-4 h-4 text-[#FDD34F]" /> Career Milestones & Recognition
                                </h4>
                                <div className="space-y-2 mb-7">
                                    {faculty.achievements.map((ach, idx) => (
                                        <div key={idx} className="flex items-center gap-3 text-xs text-white/60">
                                            <div className="w-5 h-5 rounded-lg bg-gradient-to-br from-[#F0B429]/20 to-[#804501]/20 flex items-center justify-center flex-shrink-0">
                                                <CheckCircle className="w-3 h-3 text-[#FDD34F]" />
                                            </div>
                                            {ach}
                                        </div>
                                    ))}
                                ...
                                <motion.button
                                    whileHover={{ scale: 1.02, boxShadow: '0 0 30px rgba(240,180,41,0.4)' }}
                                    whileTap={{ scale: 0.97 }}
                                    onClick={() => { onClose(); onBookSession(faculty); }}
                                    className="group w-full py-4 rounded-2xl font-black text-[#06142D] relative overflow-hidden bg-gradient-to-r from-[#F0B429] via-[#FDD34F] to-[#F0B429] shadow-[0_8px_30px_rgba(240,180,41,0.25)] transition-all"
                                >
                        ...
                        <div className="border-t border-white/8 pt-7">
                            <h4 className="text-sm font-black text-white mb-4 flex items-center gap-2">
                                <BookOpen className="w-4 h-4 text-[#FDD34F]" /> Programs & Courses Taught
                            </h4>
                            <div className="grid sm:grid-cols-2 gap-3">
                                {faculty.coursesList.map((cName, idx) => (
                                    <motion.div key={idx} whileHover={{ x: 4 }}
                                        className="group/c flex items-center justify-between p-4 rounded-2xl bg-white/[0.04] border border-white/8 hover:border-[#F0B429]/30 transition-all text-sm cursor-pointer">
                                        <span className="text-white/70 font-semibold group-hover/c:text-white transition-colors">{cName}</span>
                                        <ExternalLink className="w-3.5 h-3.5 text-white/25 group-hover/c:text-[#FDD34F] transition-colors flex-shrink-0" />
                                    </motion.div>
                                ))}
                            </div>
                        </div>
...
const FacultyPage = () => {
    ...
    return (
        <div className="min-h-screen bg-[#FAFAF8] text-[#1D2433] antialiased overflow-x-clip font-body">
            ...
            <section className="relative bg-section py-14 border-b border-[#0B1E3D]/6">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-5xl mx-auto">
                        {[
                            { icon: <GraduationCap className="w-6 h-6" />, number: "50+", label: "Master Faculty Mentors", color: "from-[#F0B429] to-[#804501]" },
                            { icon: <Users className="w-6 h-6" />, number: "12,000+", label: "Students Mentored", color: "from-[#0B1E3D] to-[#1a3a6e]" },
                            { icon: <BookOpen className="w-6 h-6" />, number: "150+", label: "Published Research Papers", color: "from-[#804501] to-[#F0B429]" },
                            { icon: <Star className="w-6 h-6" />, number: "4.95/5", label: "Average Student Rating", color: "from-[#F0B429] to-[#0B1E3D]" },
                        ].map((s, i) => (
                            <motion.div
                                key={i}
                                ...
                                whileHover={{ y: -4 }}
                                className="group flex flex-col items-center text-center p-5 rounded-2xl border border-[#0B1E3D]/8 bg-white hover:border-[#F0B429]/40 hover:shadow-lg transition-all duration-300"
                            >
                                <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${s.color} flex items-center justify-center text-[#0B1E3D] mb-4 shadow-lg group-hover:scale-110 transition-transform duration-300`}>
                                    {s.icon}
                                </div>
                    ...
                    <div className="flex items-center justify-between mb-10 pb-4 border-b border-[#0B1E3D]/8">
                        <div>
                            <p className="text-slate-500 text-sm">
                                ...
                                <span className="text-[#804501] font-bold">{selectedDepartment}</span>
                            </p>
                        </div>
                        ...
                        {filteredFaculty.length > 0 ? (
                                    ...
                                    <TiltCard key={faculty.id}>
                                        ...
                                        >
                                            ...
                                            <div className="p-6 flex flex-col flex-1">
                                                <p className="text-[#804501] text-xs font-bold uppercase tracking-widest mb-1">{faculty.department}</p>
                                                <h3 className="text-[#0B1E3D] font-black text-lg leading-snug mb-1 group-hover:text-[#804501] transition-colors duration-300">
                                                    {faculty.name}
                                                </h3>
                                                <p className="text-[#F0B429] text-xs font-bold mb-3">{faculty.designation}</p>
                                                ...
                                                <div className="flex items-center gap-5 py-4 border-y border-[#0B1E3D]/5 text-xs text-slate-500 mb-5">
                                                    <div className="flex items-center gap-1.5">
                                                        <GraduationCap className="w-3.5 h-3.5 text-[#804501]" />
                                                        {faculty.students} Students
                                                    </div>
                                                    <div className="flex items-center gap-1.5">
                                                        <BookOpen className="w-3.5 h-3.5 text-[#F0B429]" />
                                                        {faculty.papers} Papers
                                                    </div>
                                                    <div className="flex items-center gap-1.5">
                                                        <Award className="w-3.5 h-3.5 text-[#804501]" />
                            ...
                            >
                                ...
                                <motion.button
                                    ...
                                    className="px-8 py-3 rounded-2xl bg-[#0B1E3D] text-[#FDD34F] text-sm font-black shadow-lg"
                                >
                                    Reset Filters
                                </motion.button>
                            </motion.div>
